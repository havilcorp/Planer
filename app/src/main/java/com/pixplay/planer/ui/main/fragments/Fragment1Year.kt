...
class Fragment1Year: Fragment() {
    ...
    var adapter = TaskAdapter {
        (activity as MainActivity).presenter.frame_main_actionTaskMenu(FRAME.FRAME1Year, it)
    }
    ...
    fun init(rootView: View, savedInstanceState: Bundle?) {
        ...
        rootView.findViewById<FloatingActionButton>(R.id.frame_main_actionAdd).setOnClickListener {
            (activity as MainActivity).presenter.frame_main_actionAdd(FRAME.FRAME1Year)
        }
    ...
    fun updateCount() {
        (activity as MainActivity).presenter.updateCountList1Year(adapter.itemCount)
    }
